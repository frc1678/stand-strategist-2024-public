...
fun ProfileManagementScreen(navigator: DestinationsNavigator, onSwitchProfile: (String) -> Unit) {
    ...
    val actions =
        ProfileManagementActions(
            createProfile = { navigator.navigate(NewProfileDialogDestination) },
...
private fun ProfileList(shownProfile: String?, setShownProfile: (String?) -> Unit, expanded: Boolean) {
    ...
    Column(
        ...
        for (profile in appSettings.profiles) {
            Card(
                onClick = { setShownProfile(profile) },
                colors =
                ...
                Row(
                    ...
                    Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                        ...
                        // badge to indicate that the profile is currently loaded in the app
                        if (appSettings.currentProfile == profile) Badge { Text("Current") }
                    }
